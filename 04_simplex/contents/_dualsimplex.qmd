...
the dual objective function at basis $\bm{B}$.
  ...
  - The new $\bm{y}^\prime$ reamins feasible, but its objective value would
    decrease linearly in $\varepsilon$.
...
* The determination can be done by calculating the componentwise ratios
$\frac{(\bm{r_D})_j}{(-\bar{\bm{a}}^o)_j}$ for $(\bar{\bm{a}}^o)_j < 0$ and
$j = m+1, \ldots, n$ (incoming col $\bar{a}_e$).
:::
